...
def validate_num_pag(num_pag):
    if num_pag < 1 or  num_pag > 999:
...
class LivroForm(forms.ModelForm):
    ...
    class Meta:
        model = LivroModel
        fields = ['titulo', 'editora','autor','isbn','num_pag','ano']
        error_messages = {
            'titulo': {
    ...
    def clean_num_pag(self):
        num_pag = self.cleaned_data['num_pag']
        validate_num_pag(num_pag)
        return num_pag
